...
macro_rules! shared_impl {
    () => {
        ...
        pub fn is_bounded(&self) -> bool {
            self.internal.acquire_internal().capacity != usize::MAX
        }
        ...
        pub fn len(&self) -> usize {
            self.internal.acquire_internal().queue.len()
        }
        ...
        pub fn is_empty(&self) -> bool {
            self.internal.acquire_internal().queue.is_empty()
        }
        ...
        pub fn capacity(&self) -> usize {
            self.internal.acquire_internal().capacity
        }
        ...
        pub fn receiver_count(&self) -> u32 {
            self.internal.acquire_internal().recv_count
        }
        ...
        pub fn sender_count(&self) -> u32 {
            self.internal.acquire_internal().send_count
        }
...
/// Sender can be cloned and produce senders to operate in both sync and
/// async modes.
#[repr(C)]
pub struct Sender<T> {
    internal: Internal<T>,
...
impl<T> Sender<T> {
    ...
    pub fn send(&self, data: T) -> Result<(), SendError<T>> {
        ...
        if internal.recv_count == 0 {
            ...
            return Err(SendError::ReceiveClosed(data));
        }
        if let Some(first) = internal.next_recv() {
            ...
            drop(internal);
            if !sig.wait() {
                // Safety: data failed to move, returning data to caller
                return Err(SendError::Closed(unsafe { data.assume_init() }));
            }
            Ok(())
        }
    ...
    pub fn send_timeout(&self, data: T, duration: Duration) -> Result<(), SendErrorTimeout<T>> {
        ...
        if internal.recv_count == 0 {
            ...
            drop(internal);
            if send_count == 0 {
                return Err(SendErrorTimeout::Closed(data));
            }
            return Err(SendErrorTimeout::ReceiveClosed(data));
        }
        if let Some(first) = internal.next_recv() {
            drop(internal);
            // Safety: it's safe to send to owned signal once
            ...
            if !sig.wait_timeout(deadline) {
                if sig.is_terminated() {
                    // Safety: data failed to move, returning data to caller
                    return Err(SendErrorTimeout::Closed(unsafe { data.assume_init() }));
                }
                {
                    let mut internal = self.internal.acquire_internal();
                    if internal.cancel_send_signal(&sig) {
                        return Err(SendErrorTimeout::Timeout(unsafe { data.assume_init() }));
                    }
                }
    ...
    /// ```
    #[inline(always)]
    pub fn send_async(&'_ self, data: T) -> SendFuture<'_, T> {
    ...
    pub fn is_disconnected(&self) -> bool {
        self.internal.acquire_internal().recv_count == 0
    }
...
impl<T> Receiver<T> {
    ...
    /// # let (s, r) = kanal::bounded(0);
    /// # co(async move {
    /// #      s.send_async("Buddy").await?;
    /// #      anyhow::Ok(())
    /// # });
    /// let name=r.recv_async().await?;
    /// println!("Hello {}",name);
    /// # anyhow::Ok(())
    /// # });
    /// ```
    #[inline(always)]
    pub fn recv_async(&'_ self) -> ReceiveFuture<'_, T> {
    ...
    /// ```
    pub fn is_disconnected(&self) -> bool {
        self.internal.acquire_internal().send_count == 0
    }
...
pub fn bounded<T>(size: usize) -> (Sender<T>, Receiver<T>) {
    let (tx, rx) = ChannelInternal::channel(true, size);
    (Sender { internal: tx }, Receiver { internal: rx })
}
...
pub fn unbounded<T>() -> (Sender<T>, Receiver<T>) {
    let (tx, rx) = ChannelInternal::channel(false, UNBOUNDED_STARTING_SIZE);
    (Sender { internal: tx }, Receiver { internal: rx })
}
